...
contract UGForgeSmith is ReentrancyGuard, Pausable, Ownable {
    ...
    function getStakedForgeIDsForUser(address user) public view returns (uint256[] memory) {
        ...
        uint256 count;
        uint256 ttlForges = ugNFT.ttlForges();
        for (uint256 i = 1; count < numStakedForges && i <= ttlForges; i++) {
    ...
    function calculateForgeBloodReward(uint256 tokenId) public view returns (uint256 owed) {
        ...
        if (_bloodPerLevel > stake.bloodPerLevel) {
            owed += (forge.level) * (_bloodPerLevel - stake.bloodPerLevel) / 100000;
        }
    }
    ...
    function calculateStakingRewards(uint256 tokenId) public view returns (uint256, uint256) {
        ...
        if (stakedForge.stakeTimestamp > 0) {
            ...
            uint256 claimTime =
                claimTilTime > stakedForge.stakeTimestamp ? claimTilTime - stakedForge.stakeTimestamp : 0;
            ...
            if (forge.level % 7 == 0) {
                ...
                owed = (forge.level % 7) * claimTime / 1 days;
            }
    ...
    function calculateAllStakingRewards(uint256[] memory tokenIds)
        external
        view
        returns (uint256[] memory weapons, uint256[] memory amounts)
    {
        (weapons, amounts) = _calculateAllStakingRewards(tokenIds);
        return (weapons, amounts);
    }

    function _calculateAllStakingRewards(uint256[] memory tokenIds)
        private
        view
    ...
    function getBloodRepairFee(uint256 tokenId) external view returns (uint256) {
        if (tokenId % 5 == 0) {
            ...
            return BASE_REPAIR_FEE * (2 ** ((tokenId % 5) - 1)) * (tokenId / 5);
        }
    }
    ...
    function claimAllStakingRewards(address user) external whenNotPaused {
        ...
        for (uint256 i; i < _stakedForgeIds.length; i++) {
            ...
            //blood reward section
            if (_bloodPerLevel > stake.bloodPerLevel) {
                owed += (forges[i].level) * (_bloodPerLevel - stake.bloodPerLevel) / 100000;
            }
            // Just claim rewards
            Stake memory myStake;
    ...
    function equipWeapons(uint256[] calldata tokenIds, uint256[] memory weapons)
        external
        nonReentrant
        whenNotPaused
        onlyEOA
    {
        ...
        uint256 count;
        for (uint256 i = 0; i < idsToMint.length && count < unEquipCount; i++) {
            if (idsToMint[i] > 0) {
    ...
    function unEquipWeapons(uint256[] calldata tokenIds, uint256[] memory weapons)
        external
        nonReentrant
        whenNotPaused
        onlyEOA
    ...
    function upgradeWeapons(uint256[] calldata tokenIds, uint256[] calldata amounts)
        external
        nonReentrant
        whenNotPaused
        onlyEOA
    {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            if (upgradedWeapons[i] % 5 == 0) {
                //if katana
                totalBloodRepairFee += BASE_REPAIR_FEE * 16 * (upgradedWeapons[i] / 5) * upgradedAmounts[i];
            } else {
                totalBloodRepairFee += BASE_REPAIR_FEE * (2 ** ((upgradedWeapons[i] % 5) - 1))
                    * (upgradedWeapons[i] / 5) * upgradedAmounts[i];
            }
        }
    ...
    function repairWeapons(uint256[] calldata tokenIds, uint256[] calldata amounts)
        external
        whenNotPaused
        nonReentrant
        onlyEOA
    {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            if (repairedWeapons[i] % 5 == 0) {
                //if katana
                totalBloodRepairFee += BASE_REPAIR_FEE * 16 * (repairedWeapons[i] / 5) * repairedAmounts[i];
            } else {
                totalBloodRepairFee += BASE_REPAIR_FEE * (2 ** ((repairedWeapons[i] % 5) - 1))
                    * (repairedWeapons[i] / 5) * repairedAmounts[i];
            }
        }
        burnBlood(_msgSender(), totalBloodRepairFee);
        ugWeapons.batchBurn(_msgSender(), tokenIds, amounts);
    ...
    function _payForgeBloodRewards(uint256 amount) private {
        if (totalForgeLevelStaked == 0) {
            // if there's no staked Forges
    ...
    function addToTotalForgeLevelStaked(uint256 levelsToAdd) external onlyAdmin {
     ...
     */
    function setContracts(address _ugnft, address _ugFYakuza, address _blood, address _ugWeapons, address _ugArena)
        external
        onlyOwner
    {
        ugNFT = IUGNFT(_ugnft);
    ...
    function setBaseRepairFee(uint256 newFee) external onlyOwner {
