...
contract UGFightClubAlley is Ownable, Pausable, ReentrancyGuard {
    ...
    constructor(address _ugnft, address _blood, address _randomizer, address _devWallet) {
        ugNFT = IUGNFT(_ugnft);
    ...
    function getStakedFightClubIDsForUser(address user) external view returns (uint256[] memory) {
        ...
        uint256 count;
        for (uint256 i = 1; count < numStakedFightClubs; i++) {
    ...
    function _stakeFightclubs(address account, uint256[] calldata tokenIds, IUGNFT.ForgeFightClub[] memory fightclubs)
        private
    {
    ...
    function _calculateStakingRewards(uint256 tokenId) private view returns (uint256) {
        ...
        if (bloodPerLevel > myStake.bloodPerLevel) {
            owed = (fightclub.level) * (fightclub.size) * (bloodPerLevel - myStake.bloodPerLevel);
        }
        return owed;
    }
    ...
    function _payFightClubs(uint256 amount) private {
        if (totalLevelsStaked == 0) {
            // if there's no staked FightClubs
    ...
    function setContracts(address _ugNFT, address _uBlood, address _randomizer) external onlyOwner {
        ugNFT = IUGNFT(_ugNFT);
