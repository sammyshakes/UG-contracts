...
contract UGYakDen is Ownable, ReentrancyGuard, Pausable {
    ...
    function _calculateStakingRewards(uint256 tokenId, IUGFYakuza.FighterYakuza memory yakuza)
        private
        view
    ...
    function _claimYakuza(uint256 tokenId, bool unstake, IUGFYakuza.FighterYakuza memory yakuza)
        private
        returns (uint256 owed)
    {
    ...
    function rankUpYakuzas(uint256[] calldata _tokenIds, uint256[] calldata _ranksToUpgrade, bool _isStaked)
        external
        whenNotPaused
        onlyEOA
    ...
    function getYakuzaRankUpBloodCost(uint16 currentRank, uint256 ranksToUpgrade)
        public
        view
        returns (uint256 totalBloodCost)
    {
    ...
    function _addToOwnerStakedTokenList(address _address, uint256[] memory _tokenIds) internal {
        ...
        uint256 newIndex = getValueInBin(userTotalBalances[_address], USER_TOTAL_BALANCES_BITS_SIZE, YAKUZA_INDEX);
        uint256 bin;
        uint256 index;
        for (uint256 i; i < _tokenIds.length; i++) {
    ...
    function tokenOfOwnerByIndex(address _owner, uint256 _index) public view virtual returns (uint256) {
        require(
            _index < getValueInBin(userTotalBalances[_owner], USER_TOTAL_BALANCES_BITS_SIZE, YAKUZA_INDEX),
