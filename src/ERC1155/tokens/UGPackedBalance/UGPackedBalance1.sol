...
contract UGPackedBalance is IERC1155, ERC165 {
  ...
  mapping (address =>  mapping(uint256 => uint256)) internal _ownerTokenList;

  //map tokenIds to owned indexes
  mapping(uint256 => uint256) internal _ownerTokenIndexes;
  //and for all tokens list
  mapping(uint256 => uint256) internal _allTokenIndexes;
  ...
  {
    return getValueInBin(userTotalBalances[_owner], USER_TOTAL_BALANCES_BITS_SIZE, 1);
  }
  ...
  //ENUMERATION
  function _addToOwnerTokenList(address _address, uint256[] memory _tokenIds)
    internal
  {    
    
    uint newIndex = _balanceOf(_address);
    uint bin;
    uint index;
    for(uint i; i< _tokenIds.length;i++){
      ...
      index = newIndex % 16;
      // Update balance
      _ownerTokenList[_address][bin] = _viewUpdateBinValue(_ownerTokenList[_address][bin], 16, index, _tokenIds[i], Operations.Add);
      //update owner index
      bin = _tokenIds[i]/16;
      index = _tokenIds[i] % 16;
      //_updateOwnerTokenIndex(_address, _tokenId, _ownerIndex, _operation);
      _ownerTokenIndexes[bin] = _viewUpdateBinValue(_ownerTokenIndexes[bin], 16, index, newIndex, Operations.Add);
      newIndex++;
    }
    
  }

  function _removeFromOwnerTokenList(address _from, uint256[] memory _tokenIds)
    internal
  {    
    ...
    for(uint i; i< _tokenIds.length;i++){
    //16 tokenIds per uint
    lastTokenId = getValueInBin(_ownerTokenList[_from][lastTokenIndex / 16], 16, lastTokenIndex % 16);

    // console.log('lastTokenId',lastTokenId);
    // console.log('lastTokenIndex',lastTokenIndex);
    // console.log('toobi lastTokenIndex',tokenOfOwnerByIndex(_from, lastTokenIndex));
    // console.log('dbl chk lastTokenIndex',getValueInBin(_ownerTokenIndexes[lastTokenId/16], 16, lastTokenId%16));
    // console.log('_tokenIds[i] (token to remove)',_tokenIds[i]);
     
    //get token to be removed index
    bin = (_tokenIds[i])/ 16;
    index = (_tokenIds[i]) % 16;
    removeTokenIndex = getValueInBin(_ownerTokenIndexes[bin], 16, index);  
   ...
   if (_tokenIds[i] != lastTokenId) {           
      ...
      _ownerTokenList[_from][removeTokenIndex/16] = _viewUpdateBinValue(_ownerTokenList[_from][removeTokenIndex/16], 16, removeTokenIndex%16, amount, _operations);
      uint256 indexDiff = lastTokenIndex - removeTokenIndex;  

      //update new index for moved token by subtracting indexDiff
      _ownerTokenIndexes[(lastTokenId)/16] = _viewUpdateBinValue(_ownerTokenIndexes[(lastTokenId)/16], 16, (lastTokenId)%16, indexDiff, Operations.Sub);
    }     
    // This also deletes the contents at the last position
    _ownerTokenIndexes[_tokenIds[i]/16] = _viewUpdateBinValue(_ownerTokenIndexes[_tokenIds[i]/16], 16, _tokenIds[i]%16, getValueInBin(_ownerTokenIndexes[(_tokenIds[i])/16], 16, (_tokenIds[i])%16), Operations.Sub);
  
    _ownerTokenList[_from][lastTokenIndex / 16] = _viewUpdateBinValue(_ownerTokenList[_from][lastTokenIndex / 16], 16, lastTokenIndex % 16,getValueInBin(_ownerTokenList[_from][lastTokenIndex/16], 16, lastTokenIndex%16), Operations.Sub);
  ...
  function walletOfOwner(address owner) public view returns (uint256[] memory) {
    uint256 ownerTokenCount = _balanceOf(owner);
  ...
  function tokenOfOwnerByIndex(address owner, uint256 _index) public view virtual returns (uint256) {
        require(_index < _balanceOf(owner), "owner index out of bounds");

        uint256 bin = _index/16;
        uint256 index = _index % 16;
        return getValueInBin(_ownerTokenList[owner][bin], 16, index);
    }
