...
interface IUGMigration {
    function migrateRingAmulet(uint256[] calldata tokenIds, bool isRing)
        external;
...
contract UGNFTsTest is DSTest {
    ...
    function batchMintFighters(
        ...
        //uint numFighters = 44;
        IUGFYakuza.FighterYakuza[] memory FY = new IUGFYakuza.FighterYakuza[](
            numFighters
        );
    ...
    function testMintBatchFighters() public {
        ...
        uint256 _bal = raidEntry.enterRaid(
            IdsUser,
            createRaiderEntries(
        ...
        for (uint256 i; i < 2; i++) {
            bal = ugGame.levelUpFightClubs(
                IdsUser,
                createAmountsArray(IdsUser.length, 1, 0, 2),
                createAmountsArray(IdsUser.length, 1, 0, 2),
                true
            );
        ...
        for (uint256 i; i < 2; i++) {
            bal = ugGame.levelUpFightClubs(
                IdsUser,
                createAmountsArray(IdsUser.length, 1, 0, 1),
                createAmountsArray(IdsUser.length, 0, 0, 1),
                true
            );
            console.log("blood cost to level up fight clubs user 1 ", bal);
        }
        ...
        uint256 val = fclubAlley.totalFightClubsStaked();
    ...
    function createBoolArray(uint256 length, bool value)
        private
        pure
        returns (bool[] memory)
    {
    ...
    function createUint8AmountsArray(
        uint256 length,
        uint8 value,
        uint8 valueIncrement,
        uint8 modulo
    ...
    function createUint32AmountsArray(
        uint256 length,
        uint32 value,
        uint8 valueIncrement,
        uint8 modulo
    ...
    function createAmountsArray(
        ...
        for (uint256 i; i < length; i++) {
            amounts[i] =
                value +
                ((i * valueIncrement) % modulo) +
                valueIncrement;
        }
        return amounts;
    }

    function createRaiderEntries(
        uint8[] memory size,
        uint8[] memory yakFamilies,
        uint32[] memory sweat
    ) private pure returns (IRaidEntry.RaiderEntry[] memory) {
        IRaidEntry.RaiderEntry[] memory entries = new IRaidEntry.RaiderEntry[](
            yakFamilies.length
        );
        for (uint256 i; i < yakFamilies.length; i++) {
