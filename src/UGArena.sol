...
contract UGArena is IUGArena, Ownable, ReentrancyGuard, Pausable {
    ...
    event TokensClaimed(
        address indexed owner, uint256[] indexed tokenIds, bool unstaked, uint256 earned, uint256 timestamp
    );
    ...
    function calculateStakingRewards(uint256 tokenId) external view returns (uint256 owed) {
        ...
        (uint256 ringLevel, uint256 ringExpireTime, uint256 extraAmuletDays, uint256 amuletExpireTime) =
            getAmuletRingInfo(myStake.owner);
        return _calculateStakingRewards(
            tokenId, ringLevel, ringExpireTime, extraAmuletDays, amuletExpireTime, unPackFighter(fighters[0])
        );
    }
    ...
    function calculateAllStakingRewards(uint256[] memory tokenIds) external view returns (uint256 owed) {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            owed += _calculateStakingRewards(
                tokenIds[i], ringLevel, ringExpireTime, extraAmuletDays, amuletExpireTime, unPackFighter(fighters[i])
            );
        }
        return owed;
    }
    ...
    function _calculateStakingRewards(
        ...
        if (fighter.isFighter) {
            ...
            if (fighterExpireTime > myStake.stakeTimestamp) {
                owed +=
                    (fighterExpireTime - myStake.stakeTimestamp) * fighter.level * DAILY_BLOOD_RATE_PER_LEVEL / 1 days;
            }
            ...
            if (ringExpireTime >= myStake.stakeTimestamp) {
                owed += (ringExpireTime - myStake.stakeTimestamp)
                    * (fighter.level * (ringLevel * RING_DAILY_BLOOD_PER_LEVEL)) / 1 days;
            }
        }
        return (owed);
    }

    function getAmuletRingInfo(address user)
        public
        view
        returns (uint256 ringLevel, uint256 ringExpireTime, uint256 extrAmuletDays, uint256 amuletExpireTime)
    {
        ...
        if (ring > 0) {
            ...
            if (stakedRing.lastLevelUpgradeTime + 7 days < block.timestamp) {
                ...
                ringExpireTime = block.timestamp;
            }
        } else {
            ringLevel = 0;
            ringExpireTime = 0;
        ...
        if (amulet > 0) {
            ...
            if (block.timestamp > stakedAmulet.lastLevelUpgradeTime + 7 days) {
                ...
                amuletExpireTime = block.timestamp;
            }
        } else {
            extrAmuletDays = 0;
            amuletExpireTime = 0;
    ...
    function _claimManyFromArena(
        ...
        if (owed > 0) {
            ...
            if (normalizedSupply < MAXIMUM_BLOOD_SUPPLY) {
                if (normalizedSupply + owed > MAXIMUM_BLOOD_SUPPLY) {
                    // If totalSupply + owed exceeds the maximum supply then pay out only the remainder
    ...
    function _claimFighter(
        ...
        if (unstake) {
            if (randomizer.getSeeds(tokenId, owed, 1)[0] % 2 == 1) {
                // 50% chance of all $BLOOD stolen
                _payYakuzaTax(owed);
     ...
     */
    function _updateIDStakedBalance(address _address, uint256 _id, uint256 _amount, Operations _operation) internal {
        uint256 bin;
        uint256 index;
        ...
        // Update balance
        stakedBalances[_address][bin] =
            _viewUpdateBinValue(stakedBalances[_address][bin], IDS_BITS_SIZE, index, _amount, _operation);
    }
    ...
    {
        // Update balance
        userTotalBalances[_address] =
            _viewUpdateBinValue(userTotalBalances[_address], USER_TOTAL_BALANCES_BITS_SIZE, _index, _amount, _operation);
    }
     ...
     */
    function getValueInBin(uint256 _binValues, uint256 bitsize, uint256 _index) public pure returns (uint256) {
    ...
    function _viewUserStakedIdBalance(address _address, uint256 _id) internal view returns (uint256) {
        uint256 bin;
        uint256 index;
    ...
    //ENUMERATION
    function _addToOwnerStakedTokenList(address _address, uint256[] memory _tokenIds) internal {
        //get balance of fighters
        uint256 newIndex = getValueInBin(userTotalBalances[_address], USER_TOTAL_BALANCES_BITS_SIZE, FIGHTER_INDEX);
        uint256 bin;
        uint256 index;
        for (uint256 i; i < _tokenIds.length; i++) {
            ...
            index = newIndex % 16;
            // Update balance
            _ownerStakedTokenList[_address][bin] =
                _viewUpdateBinValue(_ownerStakedTokenList[_address][bin], 16, index, _tokenIds[i], Operations.Add);
            //update owner index
            bin = _tokenIds[i] / 16;
            index = _tokenIds[i] % 16;
            //_updateOwnerTokenIndex(_address, _tokenId, _ownerIndex, _operation);
            _ownerStakedTokenIndexes[bin] =
                _viewUpdateBinValue(_ownerStakedTokenIndexes[bin], 16, index, newIndex, Operations.Add);
            newIndex++;
        }
    }
    ...
    function _removeFromOwnerStakedTokenList(address _from, uint256[] memory _tokenIds) internal {
        uint256 lastTokenIndex =
            getValueInBin(userTotalBalances[_from], USER_TOTAL_BALANCES_BITS_SIZE, FIGHTER_INDEX) - 1;
        //16 tokenIds per uint
        uint256 bin;
        uint256 index;
        ...
        for (uint256 i; i < _tokenIds.length; i++) {
            ...
            if (_tokenIds[i] != lastTokenId) {
                ...
                _ownerStakedTokenList[_from][removeTokenIndex / 16] = _viewUpdateBinValue(
                    _ownerStakedTokenList[_from][removeTokenIndex / 16], 16, removeTokenIndex % 16, amount, _operations
                );
                uint256 indexDiff = lastTokenIndex - removeTokenIndex;
                //update new index for moved token by subtracting indexDiff
                _ownerStakedTokenIndexes[(lastTokenId) / 16] = _viewUpdateBinValue(
                    _ownerStakedTokenIndexes[(lastTokenId) / 16], 16, (lastTokenId) % 16, indexDiff, Operations.Sub
                );
            }
            // This also deletes the contents at the last position of the array
            _ownerStakedTokenIndexes[_tokenIds[i] / 16] = _viewUpdateBinValue(
                _ownerStakedTokenIndexes[_tokenIds[i] / 16],
                16,
                _tokenIds[i] % 16,
                getValueInBin(_ownerStakedTokenIndexes[(_tokenIds[i]) / 16], 16, (_tokenIds[i]) % 16),
                Operations.Sub
            );
            // console.log('removeTokenIndex',removeTokenIndex);
            _ownerStakedTokenList[_from][lastTokenIndex / 16] = _viewUpdateBinValue(
                _ownerStakedTokenList[_from][lastTokenIndex / 16],
                16,
                lastTokenIndex % 16,
                getValueInBin(_ownerStakedTokenList[_from][lastTokenIndex / 16], 16, lastTokenIndex % 16),
                Operations.Sub
            );
    ...
    function tokenOfOwnerByIndex(address owner, uint256 _index) public view virtual returns (uint256) {
        require(
            _index < getValueInBin(userTotalBalances[owner], USER_TOTAL_BALANCES_BITS_SIZE, FIGHTER_INDEX),
     ...
     */
    function setContracts(address _ugnft, address _ugFYakuza, address _blood, address _randomizer, address _ugGame)
        external
        onlyOwner
    {
        ugNFT = IUGNFT(_ugnft);
