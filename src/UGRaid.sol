...
contract UGRaid is IUGRaid, Ownable, ReentrancyGuard {
    ...
    function referee(uint256 maxRaiders) external nonReentrant onlyEOA {
                    ...
                    for (uint8 k; k < tempVal; k++) {
                        ...
                        for (uint256 n = 0; n < raidSize; n++) {
                            ...
                            if (!yakuzaRoundActive || raidTickets[n].yakuzaFamily > 0) {
                                ...
                                //calculate scores
                                scores[n] = (BRUTALITY_WEIGHT * raidTickets[n].fighterLevel * raidTickets[n].brutality)
                                    / (i * 3) + WEAPONS_WEIGHT * weaponsScore
                        ...
                        _quickSort(scores, raidTickets, int256(0), int256(raidTickets.length - 1));
    ...
    function getRaiderSizeQueueLengths(uint256 sizeTier) external view returns (uint256) {
        require(sizeTier > 0, "sizeTier must not be zero");
        uint256 queue;
        uint256 filledRaids;
        for (uint256 i = 1; i <= maxRaiderQueueLevelTier; i++) {
            queue = _getQueueLength(RaiderQueue[i][sizeTier]);
            filledRaids += queue / (sizeTier * 5);
        }
        return (filledRaids);
    }
    ...
    function _weaponScore(uint256 attackScore, uint256 seed) private pure returns (uint256 score, bool _isBroken) {
    ...
    function _getWeaponsScore(Raid memory raid, RaidEntryTicket memory ticket)
        private
        view
        returns (uint256 weaponsScore, RaidEntryTicket memory)
    {
        uint256 _maxWeapons;
        ...
        if (raid.levelTier >= KNUCKLES_TIER) {
            _maxWeapons++;
            (uint256 tempScore, bool _isBroken) = _weaponScore(ticket.knuckles, seeds[0]);
        ...
        if (raid.levelTier >= CHAINS_TIER) {
            _maxWeapons++;
            (uint256 tempScore, bool _isBroken) = _weaponScore(ticket.chains, seeds[1]);
            weaponsScore += tempScore;
            //add 1 to score if broken -  memory instance will record to storage after raid calcs are made
        ...
        if (raid.levelTier >= BUTTERFLY_TIER) {
            _maxWeapons++;
            (uint256 tempScore, bool _isBroken) = _weaponScore(ticket.butterfly, seeds[2]);
            weaponsScore += tempScore;
            //add 1 to score if broken -  memory instance will record to storage after raid calcs are made
        ...
        if (raid.levelTier >= MACHETE_TIER) {
            _maxWeapons++;
            (uint256 tempScore, bool _isBroken) = _weaponScore(ticket.machete, seeds[3]);
            weaponsScore += tempScore;
            //add 1 to score if broken -  memory instance will record to storage after raid calcs are made
        ...
        if (raid.levelTier >= KATANA_TIER) {
            _maxWeapons++;
            (uint256 tempScore, bool _isBroken) = _weaponScore(ticket.katana, seeds[4]);
            weaponsScore += tempScore;
            //add 1 to score if broken -  memory instance will record to storage after raid calcs are made
    ...
    function _calculateRaiderRewards(uint256[] memory scores, RaidEntryTicket[] memory raidTickets, Raid memory raid)
        private
        returns (RaidEntryTicket[] memory, bool yakShareStolen)
    {
        address raiderOwner;
        ...
        for (uint256 o; o < raidTickets.length; o++) {
    ...
    function claimWeapons() external nonReentrant onlyEOA {
        ...
        for (uint256 i; i <= 4; i++) {
            ...
            for (uint256 j = 1; j <= 4 && count < ttlWeaponTypes; j++) {
    ...
    function _assignRaidersToRaid(Raid memory raid, uint8 yakuzaFamilyWinner)
        private
        returns (RaidEntryTicket[] memory, Raid memory)
    {
        ...
        for (uint256 i; i < _raidSize; i++) {
            ...
            if (yakuzaRoundActive) {
                ...
                if (_yakScore == 0) {
                    uint256 roll = randomizer.getSeeds(raid.id, tickets[i].fighterId, 1)[0] % 100;
            ...
            if (viewIfRaiderIsInQueue(tickets[i].fighterId)) {
                _updateIfRaiderIsInQueue(tickets[i].fighterId, Operations.Sub);
            } else {
                _yakScore = 0;
            }
    ...
    function addTicketsToRaiderQueue(uint256[] memory packedTickets) external onlyAdmin {
        uint8 levelTier;
        uint8 sizeTier;
        for (uint256 i; i < packedTickets.length; i++) {
    ...
    function _quickSort(uint256[] memory keyArr, RaidEntryTicket[] memory dataArr, int256 left, int256 right)
        private
        pure
    {
        int256 i = left;
        int256 j = right;
        if (i == j) return;
        uint256 pivot = keyArr[uint256(left + (right - left) / 2)];
        while (i <= j) {
            while (keyArr[uint256(i)] > pivot) i++;
            while (pivot > keyArr[uint256(j)]) j--;
            if (i <= j) {
                (keyArr[uint256(i)], keyArr[uint256(j)]) = (keyArr[uint256(j)], keyArr[uint256(i)]);
                (dataArr[uint256(i)], dataArr[uint256(j)]) = (dataArr[uint256(j)], dataArr[uint256(i)]);
                i++;
                j--;
            }
        }
    ...
    function packTicket(RaidEntryTicket memory _ticket) private pure returns (uint256) {
    ...
    function unpackTicket(uint256 packedTicket) private pure returns (RaidEntryTicket memory _ticket) {
