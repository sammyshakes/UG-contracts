...
contract UGNFT is UGMintBurnPackedBalance, IUGNFT, Ownable {
    ...
    event LevelUpForgeFightClub(
        uint256 indexed tokenId, uint256 timestamp, uint256 indexed level, uint256 indexed size
    );
    ...
    function getNftIDsForUser(address user, uint256 nftIndex) external view returns (uint256[] memory) {
        ...
        uint256 count;
        for (uint256 i = 1; count < num && i <= ttlNfts; i++) {
    ...
    function mintRingAmulet(address _to, uint256 _level, bool isRing) external onlyAdmin {
        uint256 _id;
        if (isRing) {
    ...
    function mintFightClubForge(address _to, bytes memory _data, uint256 _size, uint256 _level, bool isFightClub)
        external
        onlyAdmin
    {
        uint256 _id;
    ...
    function levelUpRingAmulets(uint256 tokenId, uint256 newLevel) external onlyAdmin {
        RingAmulet memory traits;
    ...
    function safeTransferFrom(address _from, address _to, uint256 _id, uint256 _amount, bytes memory _data)
        public
        override (UGPackedBalance, IUGNFT)
    {
    ...
    function uri(uint256 tokenId) external view returns (string memory) {
        ...
        if (tokenId > RING && tokenId <= RING + ttlRings) {
            ...
            if (traits.lastLevelUpgradeTime != 0) {
                jsonString = string(
                    abi.encodePacked(
                        jsonString, Strings.toString(traits.level), ",", Strings.toString(traits.lastLevelUpgradeTime)
                    )
                );
            }

            _uri = string(abi.encodePacked(baseURI, "ring/ring.png", "?traits=", jsonString));
        }
        ...
        if (tokenId > AMULET && tokenId <= AMULET + ttlAmulets) {
            ...
            _uri = string(abi.encodePacked(baseURI, "amulet/amulet.png", "?traits=", jsonString));
        }

        //if forge or fight club
        if (
            (tokenId > FORGE && tokenId <= FORGE + ttlForges)
            ...
            _uri = string(abi.encodePacked(baseURI, imageId, ".png", "?traits=", jsonString));
        }
    ...
    function batchBurn(address _from, uint256[] memory _ids, uint256[] memory amounts, uint256 _index)
        external
        onlyAdmin
    {
     ...
     */
    function supportsInterface(bytes4 _interfaceID) public pure virtual override (UGPackedBalance) returns (bool) {
        if (_interfaceID == 0xd9b67a26 || _interfaceID == 0x0e89341c) {
            return true;
        }
