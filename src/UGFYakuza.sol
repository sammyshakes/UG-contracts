...
contract UGFYakuza is UGMintBurnPackedBalance, IUGFYakuza, Ownable {
    ...
    /*///////////////////////////////////////////////////////////////
                    FIGHTER MIGRATION
    //////////////////////////////////////////////////////////////*/
    function batchMigrateFYakuza(address _to, uint256[] calldata v1TokenIds, FighterYakuza[] calldata oldFighters)
        external
        onlyAdmin
    {
        ...
        uint256 newFighter;
        uint256 nextVal;
        for (uint256 i; i < oldFighters.length; i++) {
    ...
    function setRaidTraitsFromPacked(uint256[] memory packedTickets) external onlyAdmin {
        ...
        FighterYakuza memory FY;
        for (uint256 i = 0; i < packedTickets.length; i++) {
            ticket = unpackTicket(packedTickets[i]);

            if (ticket.fighterId > FIGHTER + ttlFYakuzas || ticket.fighterId <= FIGHTER) {
                revert InvalidTokenID({tokenId: ticket.fighterId});
            }
    ...
    function getPackedFighters(uint256[] calldata tokenIds) external view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            if (tokenIds[i] > FIGHTER + ttlFYakuzas || tokenIds[i] <= FIGHTER) {
                revert InvalidTokenID({tokenId: tokenIds[i]});
            }
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        ...
        if (tokenId > FIGHTER && tokenId <= FIGHTER + ttlFYakuzas) {
            ...
            _uri = string(abi.encodePacked(baseURI, fyakuza, imageId, ".png", "?traits=", jsonString));
        }
        return _uri;
    }
    ...
    function unpackTicket(uint256 packedTicket) private pure returns (iUGRaid.RaidEntryTicket memory _ticket) {
